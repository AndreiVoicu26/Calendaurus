...
namespace Calendaurus.API.Controllers
{
    ...
    public class StudentsController : ControllerBase
    {
        ...
        [HttpGet("theoretical")]
        public async Task<IActionResult> GetTheoreticalLabs()
        {
            var student = await _context.Students
                .Include(s => s.User)
                .Where(s => s.User.Email == User.Identity.Name)
            ...
            {
                var discipline = await _context.Disciplines
                    .Include(d => d.TheoreticalLesson)
                    .ThenInclude(lessons => lessons.TheoreticalLessonEvents)
                    .Where(d => d.Year == student.Year)
                    .ToListAsync();

                var theoretical = discipline.Select(d => d.TheoreticalLesson);
                var results = theoretical.Select(_mapper.Map<TheoreticalLessonApiModel>);
                return Ok(results);
            }
